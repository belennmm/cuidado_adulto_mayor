...
class FamilyCareEndpointsTest extends TestCase
{
    ...
    public function test_family_can_get_incidents_for_assigned_older_adult(): void
    {
        ...
        $this->getJson("/api/family/older-adults/{$assignedAdult->id}/incidents?date=2026-05-01")
            ...
            ->assertJsonPath('filters.older_adult_id', $assignedAdult->id)
            ->assertJsonPath('filters.date', '2026-05-01')
            ->assertJsonPath('summary.total', 1)
            ->assertJsonPath('summary.open', 1)
            ->assertJsonPath('summary.resolved', 0)
            ->assertJsonPath('older_adult.id', $assignedAdult->id)
            ->assertJsonPath('older_adult.full_name', 'Rosa Martinez')
            ->assertJsonPath('incidents.0.title', 'Revision de presion')
            ->assertJsonPath('incidents.0.description', 'Se notifico lectura elevada.')
            ->assertJsonPath('incidents.0.incident_date', '2026-05-01')
            ->assertJsonPath('incidents.0.incident_time', '08:30:00')
